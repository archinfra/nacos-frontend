...
import { parseBizTags, resolveSkillGroups, type SkillListItem } from '@/types/skill';
...
export function SkillCard({
  ...
  const groups = resolveSkillGroups(item);
  const bizTags = parseBizTags(item.bizTags)
    .filter((tag) => !groups.includes(tag) && tag !== item.skillSet)
    .slice(0, 2);
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start gap-3 px-4 pt-3.5 pb-2 relative">
        ...
        {/* Icon */}
        <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-violet-500 to-fuchsia-400 shadow-sm shadow-violet-500/15">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-1.5 mt-1 flex-wrap">
            ...
            {groups[0] && (
              <span className="inline-flex items-center gap-0.5 text-[10px] text-muted-foreground bg-muted/60 px-1 py-0.5 rounded">
