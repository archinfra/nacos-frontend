...
import { resolvePrimarySkillGroup } from '@/types/skill';
...
export default function SkillManagementPage() {
  ...
  const groupedItems = useMemo(() => {
    ...
    items.forEach((item) => {
      const group = resolvePrimarySkillGroup(item);
            ...
            {groupedItems.map(([groupName, groupItems]) => (
              <section key={groupName} className="space-y-3">
                <div className="flex items-center gap-2 text-sm font-semibold text-foreground">
                  ...
                  <span>{groupName === 'Ungrouped' ? t('skill.ungrouped') : groupName}</span>
                  <span className="text-xs font-normal text-muted-foreground">{groupItems.length}</span>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                  {groupItems.map((item) => (
                    <SkillCard
